...
declare const __TRENTCAD_ADMIN_PIN_HASH__: string

/**
 * The admin-page PIN hash baked in at build time from the
 * TRENTCAD_ADMIN_PIN_HASH GitHub Actions secret. Empty string when no
 * secret was provided (e.g. local dev builds) — in that case the admin
 * page opens without a PIN prompt.
 */
const RAW_HASH: string = (typeof __TRENTCAD_ADMIN_PIN_HASH__ !== 'undefined'
  ? __TRENTCAD_ADMIN_PIN_HASH__
  : '').toLowerCase().trim()
...
if (RAW_HASH && !HASH_VALID) {
  ...
  console.error(
    '[admin-pin] TRENTCAD_ADMIN_PIN_HASH is set but is not a valid lowercase ' +
